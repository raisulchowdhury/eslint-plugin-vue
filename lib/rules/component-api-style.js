    ...
    return utils.compositingVisitors(
      ...
      utils.defineVueVisitor(context, {
        onVueObjectEnter(node) {
          ...
          let hasApiSpecificOption = false
          for (const prop of node.properties) {
            ...
            if (apis.some((api) => api.options.has(name))) {
              hasApiSpecificOption = true
            }
            const disallowApi =
              apis.every((api) => !(api.allow && api.options.has(name))) &&
          ...
          if (isPreferScriptSetup(allows) && !hasApiSpecificOption) {
            context.report({
              node,
              messageId: 'disallowComponentOptionPreferScriptSetup',
              data: {
                disallowedApi: 'Options API'
              }
            })
          }
        }
