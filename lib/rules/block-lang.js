...
function normalizeOption(blockName, option) {
  ...
  // An explicit `allowNoLang: false` must win even when the configured
  // language is the block's default language. This allows users to require
  // `lang="js"`, `lang="html"`, or `lang="css"` when desired.
  if (option.allowNoLang === false) {
    return {
      lang,
      allowNoLang: false
    }
...
function normalizeOptions(options) {
  ...
  for (const [blockName, value] of Object.entries(options)) {
    if (value) {
